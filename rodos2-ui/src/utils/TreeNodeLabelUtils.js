...
export function getNodeLabel(node) {
    ...
    const complexName = property.complexName || '';
    const inDataType = property.inDataType || '';

    // NONE or simple type
    if (complexType === 'NONE' || complexType === 'None' || !complexType) {
        const valueStr = value ? `, ${value}` : '';
        return `(${type}) ${name}, ${description}, ${unit}${valueStr}`;
    }

    // CLASS type
    if (complexType === 'CLASS' || complexType === 'class') {
        return `(class) ${name}, ${complexName}`;
    }

    // ARRAY type
    if (complexType === 'ARRAY' || complexType === 'array') {
        const valuesStr = formatArrayValues(value);
        return `(array) ${name}, ${type}, ${unit}, ${description}, ${valuesStr}`;
    }

    // VECTOR type
    if (complexType === 'VECTOR' || complexType === 'vector') {
        const valuesStr = formatArrayValues(value);
        return `(vector) ${name}, ${type}, ${unit}, ${description}, ${valuesStr}`;
    }

    // POINTER type
    if (complexType === 'POINTER' || complexType === 'pointer') {
        return `(pointer) ${name}, ${inDataType}`;
    }
